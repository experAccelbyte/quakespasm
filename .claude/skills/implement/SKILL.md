---
name: implement
description: Plan then execute a coding task with epistemic-integrity-guided precision
---
...
- **Clarify before building** — if the task is ambiguous, ask all clarifying questions at once before diving in.
...
Confirm the directive is loaded with a single short sentence that also states the task — e.g., "Epistemic Integrity Directive active — implementing: [task]." Do not summarize or paraphrase the directive's contents.

---

## Step 3 — Clarify if Needed

Before reading any code, evaluate whether the task is specific enough to plan correctly.
...
## Step 4 — Verify Before Planning

Before producing a plan, verify the relevant facts from the codebase:
...
- **Build impact**: Whether this requires any build system changes (CMakeLists.txt, .Build.cs, module deps, etc.)
- **Test strategy**: Which existing tests are relevant, and whether new tests need to be written
- **Risks / open questions**: Anything uncertain or potentially impactful
...
Use `ExitPlanMode` to leave plan mode, then implement the approved plan exactly. Follow the code style preferences from CLAUDE.md:
...
Make focused, atomic changes. Do not modify files outside the approved plan scope without telling the developer first.
...
- **Deviations from plan**: anything that was implemented differently from the approved plan, and why
