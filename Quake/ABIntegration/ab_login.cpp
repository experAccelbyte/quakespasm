...
AB_Login::AB_Login(ABTaskRunner& task_runner)
...
void AB_Login::LoginWithDeviceId(const char* server_url,
                                  const char* client_id,
                                  const char* client_secret)
{
    accelbyte::settings::InMemorySettings settings;
    if (server_url    && server_url[0])    settings.set_server_url(server_url);
    if (client_id     && client_id[0])     settings.set_client_id(client_id);
    if (client_secret && client_secret[0]) settings.set_client_secret(client_secret);
...
void AB_Login::OnLoginError(const accelbyte::Error& error)
{
    ...
    if (task_runner_)
        task_runner_->queue_task([](const std::string& msg) {
