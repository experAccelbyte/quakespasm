...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin, const std::map<std::string, std::string>& extra_headers = std::map<std::string, std::string>()) {
    char host[512];
    int port;
    char path[512];
    if (url.size() >= 512) {
    ...
    }
    {
        ...
        for (auto it = extra_headers.begin(); it != extra_headers.end(); ++it) {
            snprintf(line, 1024, "%s: %s\r\n", it->first.c_str(), it->second.c_str()); ::send(sockfd, line, strlen(line), 0);
        }
...
WebSocket::pointer WebSocket::from_url(const std::string& url, const std::map<std::string, std::string>& extra_headers, const std::string& origin) {
    return ::from_url(url, true, origin, extra_headers);
}
